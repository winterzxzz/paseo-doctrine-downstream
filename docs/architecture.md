...
- Owns the bundled Beads Central sidecar lifecycle; daemon/WebUI vẫn chạy ở trạng thái degraded khi
  Central unavailable, còn mọi Beads-dependent mutation/workflow fail closed tại owning service
...
- Agent state persists to `$PASEO_HOME/agents/{cwd-with-dashes}/{agent-id}.json`; canonical timeline rows persist under `$PASEO_HOME/agent-timelines/{encoded-agent-id}/`. Agent-record storage is derived from `cwd`, not from workspace id.
...
├── agent-timelines/{encoded-agent-id}/        # Durable canonical timeline rows
