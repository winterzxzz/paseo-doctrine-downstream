...
- Make sure the resolved release source passed CI, the approved release inputs are committed locally on
  the intended `main` branch, and the working tree is clean.
- Run `npm run format`, `npm run lint`, and `npm run typecheck`, then commit any resulting changes before
  starting a `release:*` command. Run `release:prepare` once on its own and commit any lockfile churn before
  the release command; `npm version` aborts if that preparation leaves the worktree dirty.
- Use the exact npm version pinned by `packageManager`. `release:check` rejects a different npm version,
  installs from the committed lockfile, and fails if any check changes tracked bytes.
- `release:prepare` is reserved for the `npm version` lifecycle. Review workspace-link and lockfile changes
  as release inputs rather than hiding them inside the version commit.
