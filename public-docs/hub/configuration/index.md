...
> **Downstream status:** the starter trigger below is the upstream v0.8.0 shape. Paseo Foundation
> retains it as an adaptation reference, but its guided command fails closed with
> `HUB_FOUNDATION_ADMISSION_REQUIRED` and writes nothing; the shape is not a role/assignment
> authority receipt.
