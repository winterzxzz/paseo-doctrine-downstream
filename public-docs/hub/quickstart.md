...
> **Downstream status:** this upstream v0.8.0 journey is reference-only in Paseo Foundation.
> `paseo hub init` returns `HUB_FOUNDATION_ADMISSION_REQUIRED` before it changes any state. Reopen
> the journey only after Hub revisions can carry role, assignment, Workspace Protocol admission,
> and exact Hub-output grants through daemon materialization.

Phần dưới ghi lại journey của upstream stable để review và thiết kế adaptation. Đừng chạy nó như một
quickstart downstream: automatic continuation sau `paseo hub login` không được attach, còn
`paseo hub init` dừng trước daemon connection, workspace read/write và deploy.

Luồng first-run hiện dùng được cho Foundation nằm tại
[Foundation first run](../../docs/foundation-first-run.md). Manual Hub login/deploy vẫn dành cho bundle
đã được Human review riêng; nó không tự cấp role, assignment hoặc output authority.

## Upstream stable reference — non-executable downstream

Upstream journey runs Hub on your machine, connects it to Slack without a public server, and answers a mention with an agent in your repository. Upstream browser setup hands off to your terminal, and `paseo hub init` writes and deploys a starter trigger for you.
