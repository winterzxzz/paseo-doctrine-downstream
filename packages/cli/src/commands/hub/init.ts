...
export const FOUNDATION_HUB_STARTER_ADMISSION_ERROR = "HUB_FOUNDATION_ADMISSION_REQUIRED";
...
export function addHubInitCommand(
  ...
  parent
    .command("init")
    .description("Check availability of the governed Foundation Hub starter")
    .action(
...
export async function runHubGuidedSetup(
  ...
  if (!foundationHubStarterAuthoritySupported()) {
    throw foundationHubStarterAdmissionError();
  }
...
async function writeScaffold(
  _cwd: string,
  _scaffold: ReturnType<typeof createHubInitScaffold>,
...
/**
 * Stable v0.5.0 Hub does not carry the approved revision/step assigner,
 * role assignment, Workspace Protocol receipt, or exact Hub output grants.
 * Keep provider and connection discovery available as adaptation source, but
 * never advertise or run the generated starter until both sides negotiate
 * that authority contract.
 */
export function foundationHubStarterAuthoritySupported(): boolean {
  return false;
}

function foundationHubStarterAdmissionError(): HubCommandError {
  return new HubCommandError(
    FOUNDATION_HUB_STARTER_ADMISSION_ERROR,
    "Guided Hub starter deployment is deferred in Paseo Foundation: stable v0.5.0 Hub cannot carry a revision-scoped role/assignment receipt through to daemon admission.",
    "No files were written and no workflow was deployed. Use manual Hub configuration only under an explicit Human-reviewed authority plan; reopen guided setup after Hub and daemon advertise the Foundation authority-contract capability.",
  );
}
