...
describe("paseo daemon bootstrap", () => {
  ...
  test("reload applies live HTTP, MCP, Git, provider, relay, and app policies", async () => {
    ...
    try {
      ...
      // MCP SDK 1.x validates Accept first (406); the downstream MCP 2.x handler
      // validates Content-Type first (415). Both prove the enabled route handled
      // the malformed request before the reload disables it with 404.
      expect([406, 415]).toContain(beforeMcp.status);
