...
export class AgentManager {
  ...
  private async flushDurableTimelineForLeadHandoffInternal(agentId: string): Promise<void> {
    ...
    try {
      ...
      await this.durableTimelineStore.getCommittedSnapshot(agentId);
    } catch (error) {
  ...
  private async reloadAgentSessionInternal(
    ...
    assertRoleBoundReloadAllowed(existing, overrides);
    const { storedConfig, launchConfig, paseoToolPolicy, roleBinding, launchContract } =
      await this.prepareSessionConfig(
        refreshConfig,
        agentId,
    ...
    let handedToRegistration = false;
    // Closing the existing agent unregisters it; keep its role binding resolvable so the
    // replacement session's tool catalog stays under the same role ceiling while it opens.
    if (roleBinding) this.roleBindingsAwaitingRegistration.set(agentId, roleBinding);
    try {
      ...
      this.publishFailedReload(existing, closedExisting, error);
      throw error;
    } finally {
      this.roleBindingsAwaitingRegistration.delete(agentId);
      if (!handedToRegistration) {
  ...
  private publishFailedReload(
    ...
    if (this.agents.get(existing.id) !== existing) return;
  ...
  async steerOrReplaceActiveTurnAuthorized(
    agentId: string,
    prompt: AgentPromptInput,
    options?: AgentSteerOptions,
  ): Promise<ActiveTurnSteerDispatchResult> {
    return withAgentAuthorityLock(agentId, async () => {
      ...
      assertAgentPromptLease(record);
      return this.steerOrReplaceActiveTurn(agentId, prompt, options);
    });
  }
  ...
  private buildManagedAgentForRegister(params: {
    ...
    return {
      ...
      historyPrimed: registration.historyPrimed ?? false,
  ...
  private async flushTasks(options: { includeAgentRegistrations: boolean }): Promise<void> {
    ...
    let durableTimelineError: unknown;
    try {
      await this.durableTimelineBuffer?.flushAll();
    } catch (error) {
      durableTimelineError = error;
      this.logger.error({ err: error }, "Failed to reconcile durable agent timelines");
    }
    ...
    if (options.includeAgentRegistrations && durableTimelineError !== undefined) {
      throw new AggregateError(
        [durableTimelineError],
...
function assertRoleBoundReloadAllowed(
  existing: Pick<ActiveManagedAgent, "roleBinding" | "launchContract">,
