...
export class ProviderSnapshotManager {
  ...
  constructor(options: ProviderSnapshotManagerOptions) {
    ...
    // Role-binding support reads client capabilities, so clients exist before the first generation.
  ...
  getAgentManagerProviderState(): AgentManagerProviderState {
    return this.createAgentManagerState(
      this.generation.definitions,
      this.providerClients,
      this.providerOverrides,
    );
  }
  ...
  private createAgentManagerState(
    ...
    for (const [provider, definition] of Object.entries(definitions)) {
      providerDefinitions[provider] = {
        ...
        roleBindingSupport: this.getRoleBindingSupport(
          provider,
          definition,
          overrides,
          providerClients,
  ...
  replacePluginProviders(
    ...
    this.createAgentManagerState(this.generation.definitions, clients, this.providerOverrides);
    ...
    const generation = this.createGeneration(definitions, this.providerOverrides, clients);
    const state = this.createAgentManagerState(definitions, clients, this.providerOverrides);
  ...
  prepareMutableProviderConfig(
    ...
    for (const provider of new Set([...this.generation.order, ...Object.keys(definitions)])) {
      ...
      // Role binding lives on the override, outside the definition's configuration identity.
      const roleBindingChanged = !isDeepStrictEqual(
        this.providerOverrides?.[provider]?.roleBinding,
        providerOverrides?.[provider]?.roleBinding,
      );
      if (
        !before ||
        !after ||
    ...
    const generation = this.createGeneration(definitions, providerOverrides, clients);
    ...
    for (const provider of order) {
      ...
      providerStates.set(provider, {
        ...
        initial: identifyEntry({
          ...
          roleBinding: this.getRoleBindingSupport(provider, definition, overrides, clients),
        }),
      });
    }
  ...
  private getRoleBindingSupport(
    ...
    const client = this.ensureClient(provider, definition, clients);
    ...
    return resolveProviderRoleBindingSupport(
      provider,
      definition.derivedFromProviderId ?? null,
      legacyRoleId,
