    ...
    try {
      expect(
        manager
          ...
          .find((entry) => entry.provider === "cursor"),
      ).toMatchObject({
        roleBinding: {
          status: "unsupported",
    ...
    try {
      ...
      expect(
        manager
          .getSnapshot()
          .records.map(({ entry }) => entry)
          .find((entry) => entry.provider === "codex-proxy")?.source,
      ).toBe("custom");
    } finally {
      manager.destroy();
    }
    ...
    try {
      ...
      expect(
        manager
          .getSnapshot()
          .records.map(({ entry }) => entry)
          .some((entry) => entry.provider === "codex-proxy"),
      ).toBe(false);
...
describe("ProviderSnapshotManager cwd routing", () => {
  ...
  test("global snapshot does not satisfy an explicit home workspace read", async () => {
    ...
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      extraClients: {
        "gemini-antigravity": createExtraClient("gemini-antigravity", {
          isAvailable: vi.fn(async () => true),
          fetchCatalog,
        }),
      },
    ...
    try {
      await manager.refreshSettingsSnapshot({ providers: ["gemini-antigravity"] });
...
test("settings refresh starts independent workspace discoveries together and shares equivalent catalogues", async () => {
  ...
  try {
    await Promise.all(
      cwds.map((cwd) =>
        manager.warmUpSnapshotForCwd({ cwd, providers: ["gemini-antigravity", "codex"] }),
      ),
    );
    expect(sharedFetches).toBe(1);
    refreshing = true;
    refresh = manager.refreshSettingsSnapshot({ providers: ["gemini-antigravity", "codex"] });
...
test("bounds each provider across workspaces without blocking another provider", async () => {
    ...
    extraClients: {
      ...
      "gemini-antigravity": createExtraClient("gemini-antigravity", {
        ...
        async fetchCatalog() {
          return {
            models: [
              { provider: "gemini-antigravity", id: refreshing ? "new" : "old", label: "Model" },
            ],
            modes: [],
          };
        },
  ...
  try {
    await Promise.all(
      cwds.map((cwd) => manager.warmUpSnapshotForCwd({ cwd, providers: ["gemini-antigravity"] })),
    );
    manager.on("change", ({ current }) => {
      if (
        current.records.find((record) => record.entry.provider === "gemini-antigravity")?.entry
          .models?.[0]?.id === "new"
      ) {
    ...
    refreshing = true;
    requests.push(manager.refreshSettingsSnapshot({ providers: ["gemini-antigravity"] }));
    await expect.poll(() => started.size).toBeGreaterThanOrEqual(4);
    requests.push(
      manager.warmUpSnapshotForCwd({ cwd: "/concurrent-read", providers: ["gemini-antigravity"] }),
    );
    expect(peak).toBe(4);
    requests.push(
...
test.each(["configuration", "shutdown", "destroy"])(
  ...
  async (action) => {
    ...
    const manager = new ProviderSnapshotManager({
      ...
      providerOverrides: {
        ...PUBLICATION_PROVIDERS,
        "gemini-antigravity": { enabled: true, label: "Initial" },
      },
      extraClients: {
        "gemini-antigravity": createExtraClient("gemini-antigravity", {
          async getCatalogCacheKey() {
    ...
    const requests = cwds.map((cwd) =>
      manager.warmUpSnapshotForCwd({ cwd, providers: ["gemini-antigravity"] }),
    );
    let transitions = 0;
    manager.on("change", () => {
      transitions++;
    ...
    try {
      ...
      if (action === "configuration") {
        manager.applyMutableProviderConfig({
          ...PUBLICATION_PROVIDERS,
          "gemini-antigravity": { enabled: true, label: "Updated" },
        });
        await expect.poll(() => queuedSettled).toBe(true);
        // Replacing configuration must not reset the budget for still-active probes.
        expect(calls).toEqual(cwds.slice(0, 4));
        release();
        await manager.warmUpSnapshotForCwd({ cwd: cwds[4], providers: ["gemini-antigravity"] });
        await Promise.all(requests);
...
test("snapshot records share provider results across targets while retaining target-specific catalogues", async () => {
  const calls = { codex: 0, "gemini-antigravity": 0 };
  const manager = new ProviderSnapshotManager({
    logger: createTestLogger(),
    extraClients: Object.fromEntries(
      (["codex", "gemini-antigravity"] as const).map((provider) => [
        provider,
        createExtraClient(provider, {
          isAvailable: async () => true,
  ...
  try {
    ...
    await Promise.all(
      targets.map((cwd) =>
        manager.listProviders({ cwd, providers: ["codex", "gemini-antigravity"], wait: true }),
      ),
    );
    ...
    expect(
      new Set(
        snapshots.map(
          ({ records }) =>
            records.find(({ entry }) => entry.provider === "gemini-antigravity")!.contentHash,
        ),
      ).size,
    ).toBe(3);
    expect(calls).toEqual({ codex: 1, "gemini-antigravity": 3 });
