...
function resolveBeadsService(
  beadsService: BeadsService | undefined,
  logger: pino.Logger,
  daemonConfigStore: DaemonConfigStore,
  paseoHome: string,
  projectRegistry: ProjectRegistry,
): BeadsService {
  if (beadsService) return beadsService;
  return new BeadsCentralService({
    logger,
...
export class VoiceAssistantWebSocketServer {
  ...
  private async broadcastAgentAttention(params: {
    ...
    for (const [ws, connection] of this.sessions) {
      if (!connection.session.allowsPermission("workspace.read")) continue;
      if (
    ...
    for (const { ws } of clientEntries) {
      ...
      if (connection?.session.allowsOutbound(message)) {
        if (connection.session.delivery.isModern(ws))
          connection.session.publishToSource(ws, message);
        else this.sendToClient(ws, wrapSessionMessage(message));
      }
    }
  }
