...
export class Session {
  ...
  // Implicit delivery bypasses owned subscriptions, so it applies the outbound permission here.
  private deliverImplicitTimelineMessage(source: object, message: SessionOutboundMessage): void {
    if (this.authorization.allowsOutbound(message)) this.onMessageToSource?.(source, message);
  }
  ...
  private forwardAgentStream(
    ...
    for (const [source, { capabilities }] of this.clientSources) {
      ...
      if (!alreadyDelivered) this.deliverImplicitTimelineMessage(source, message);
    }
    if (
  ...
  public allowsOutbound(message: SessionOutboundMessage): boolean {
    return this.authorization.allowsOutbound(message);
  }
  ...
  private dispatchWorkspaceDomainMessage(
    msg: SessionInboundMessage,
    source?: object,
  ): Promise<void> | undefined {
    return (
      this.dispatchProjectConfigurationMessage(msg) ??
      this.dispatchConfigurationMessage(msg) ??
  ...
  private dispatchAgentConfigMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    const distributionUpdate = this.daemonSession.handleDistributionUpdateRequest(msg);
    if (distributionUpdate) return distributionUpdate;
    switch (msg.type) {
  ...
  private dispatchConfigurationMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    return this.dispatchFoundationConfigurationMessage(msg) ?? this.dispatchAgentConfigMessage(msg);
  }

  private dispatchFoundationConfigurationMessage(
    msg: SessionInboundMessage,
  ): Promise<void> | undefined {
    switch (msg.type) {
  ...
  private dispatchChatMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    ...
    switch (msg.type) {
      case "chat/create":
        return session.handleChatCreateRequest(msg);
      case "chat/list":
        return session.handleChatListRequest(msg);
      case "chat/inspect":
        return session.handleChatInspectRequest(msg);
      case "chat/delete":
        return session.handleChatDeleteRequest(msg);
      case "chat/post":
        return session.handleChatPostRequest(msg);
      case "chat/read":
