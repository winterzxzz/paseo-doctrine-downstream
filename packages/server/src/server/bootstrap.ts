...
async function resolveDaemonTrustedSembleRuntime(
  dependencies: PaseoDaemonDependencies,
  paseoHome: string,
  logger: Logger,
): Promise<TrustedSembleRuntime | null> {
  if (Object.prototype.hasOwnProperty.call(dependencies, "trustedSembleRuntime")) {
    return dependencies.trustedSembleRuntime ?? null;
  }
  return await resolveTrustedSembleRuntimeForDaemon(paseoHome, logger);
}
...
function toMutableProviderConfig(
  override: ProviderOverride,
): MutableDaemonConfig["providers"][string] {
  const providerConfig: MutableDaemonConfig["providers"][string] = {};
  if (override.enabled !== undefined) providerConfig.enabled = override.enabled;
  if (override.extends !== undefined) providerConfig.extends = override.extends;
  if (override.label !== undefined) providerConfig.label = override.label;
  const publicEnvironment = Object.fromEntries(
    Object.entries(override.env ?? {}).filter(([key]) => !isProviderCredentialEnvironmentKey(key)),
  );
  if (Object.keys(publicEnvironment).length > 0) providerConfig.env = publicEnvironment;
  if (override.credentialRef !== undefined) providerConfig.credentialRef = override.credentialRef;
  if (override.additionalModels) providerConfig.additionalModels = override.additionalModels;
  if (override.paseoTools) providerConfig.paseoTools = override.paseoTools;
  return providerConfig;
}

function resolveOptionalMutableDaemonConfig(
  config: PaseoDaemonConfig,
): Partial<MutableDaemonConfig> {
  return {
...
function createInitialMutableDaemonConfig(config: PaseoDaemonConfig): MutableDaemonConfig {
  const providers: MutableDaemonConfig["providers"] = Object.fromEntries(
    Object.entries(config.providerOverrides ?? {}).map(([providerId, override]) => [
      providerId,
      toMutableProviderConfig(override),
    ]),
  );
  ...
  const initialConfig: MutableDaemonConfig = {
    ...
    ...resolveOptionalMutableDaemonConfig(config),
...
export async function createPaseoDaemon(
  ...
  const trustedSembleRuntime = await resolveDaemonTrustedSembleRuntime(
    dependencies,
    config.paseoHome,
    logger,
  );
  ...
  const initialAgentMcpAvailable = agentMcpIsAvailable(
    config.mcpEnabled,
    config.mcpInjectIntoAgents,
  );
  agentManager.setPaseoToolsEnabled(initialAgentMcpAvailable);
  setAgentProviderToolsEnabled(initialAgentMcpAvailable);
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            const applyAgentMcpConfig = () => {
              ...
              setAgentProviderToolsEnabled(agentMcpAvailable);
            };
            applyAgentMcpConfig();
