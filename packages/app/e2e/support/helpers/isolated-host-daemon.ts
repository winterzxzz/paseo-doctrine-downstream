...
async function installPublishedServer(input: {
  version: string;
  paseoHome: string;
  preserveHome: boolean;
}): Promise<string> {
  const publishedPackageRoot = await mkdtemp(path.join(tmpdir(), "paseo-e2e-published-server-"));
  try {
    await writeFile(
    ...
    execFileSync(
      ...
      [
        ...
        "--no-fund",
        "--no-package-lock",
        `@getpaseo/server@${input.version}`,
      ],
      { cwd: publishedPackageRoot, stdio: "ignore" },
    );
    return publishedPackageRoot;
  } catch (error) {
    if (!input.preserveHome) {
      await rm(input.paseoHome, REMOVE_TREE_OPTIONS);
    }
...
export async function startIsolatedHostDaemon(
  ...
  const publishedPackageRoot = options.publishedVersion
    ? await installPublishedServer({
        version: options.publishedVersion,
        paseoHome,
        preserveHome: options.preserveHome ?? false,
      })
    : null;
