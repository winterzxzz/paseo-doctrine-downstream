...
export function buildSubagentPillPresentation(
  ...
  const pendingPermissionCount = rows.reduce(
    (count, row) => count + (row.kind === "paseo" ? row.pendingPermissionCount : 0),
    0,
  );
  if (counts.length === 0 && pendingPermissionCount === 0) {
  ...
  const approvalLabel =
    pendingPermissionCount > 0
      ...
      : null;
  const segments: ComposerTrackPillSegment[] = counts.map(({ bucket }, index) => ({
    bucket,
    text: labels[index] ?? "",
  }));
  if (approvalLabel) {
    segments.unshift({ bucket: "needs_input", text: approvalLabel });
  }
  const accessibilityLabels = approvalLabel ? [approvalLabel, ...labels] : labels;
  return {
    segments,
