...
/** Coordination surfaces are host-scoped: follow the last workspace's host, else the first host. */
function useCoordinationServerId(): string | null {
  const hosts = useHosts();
  const lastWorkspaceSelection = useLastWorkspaceSelection();
  return lastWorkspaceSelection?.serverId ?? hosts[0]?.serverId ?? null;
}

function SidebarTopologyRow({ onBeforeNavigate }: SidebarNavRowProps) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const serverId = useCoordinationServerId();
  const handlePress = useCallback(() => {
    if (!serverId) return;
    onBeforeNavigate?.();
    router.push(buildHostTopologyRoute(serverId));
  }, [onBeforeNavigate, serverId]);

  return (
    <SidebarHeaderRow
      icon={Network}
      label={t(builtinSidebarNavLabelKey("topology"))}
      onPress={handlePress}
      isActive={pathname.includes("/topology")}
...
function SidebarRoomsRow({ onBeforeNavigate }: SidebarNavRowProps) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const serverId = useCoordinationServerId();
  const handlePress = useCallback(() => {
    if (!serverId) return;
    onBeforeNavigate?.();
    router.push(buildHostRoomsRoute(serverId));
  }, [onBeforeNavigate, serverId]);

  return (
    <SidebarHeaderRow
      icon={MessagesSquare}
      label={t(builtinSidebarNavLabelKey("rooms"))}
      onPress={handlePress}
...
function SidebarCouncilsRow({ onBeforeNavigate }: SidebarNavRowProps) {
  const { t } = useTranslation();
  const pathname = usePathname();
  const serverId = useCoordinationServerId();
  const handlePress = useCallback(() => {
    if (!serverId) return;
    onBeforeNavigate?.();
    router.push(buildHostCouncilsRoute(serverId));
  }, [onBeforeNavigate, serverId]);

  return (
    <SidebarHeaderRow
      icon={Scale}
      label={t(builtinSidebarNavLabelKey("councils"))}
      onPress={handlePress}
      isActive={pathname.includes("/councils")}
...
const BUILTIN_ROWS: Record<BuiltinSidebarNavId, ComponentType<SidebarNavRowProps>> = {
  ...
  topology: SidebarTopologyRow,
  rooms: SidebarRoomsRow,
  councils: SidebarCouncilsRow,
