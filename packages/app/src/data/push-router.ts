...
interface ServerDataPushClient {
  ...
  // Council case updates are broadcast, not a subscribable event type.
  on<TType extends "council.case.updated">(
    type: TType,
...
export function mountServerDataPushRouter(input: PushRouterInput): () => void {
  ...
  // Council case updates are broadcast outside the session event subscription set.
